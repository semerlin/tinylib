...
static void t_keyfile_free_hash_node(void *data)
{
    ...
    kv_node *kv = T_CONTAINER_OF(hash_node, kv_node, node);
    free(kv->value);
    free(kv);
}
...
static void t_keyfile_free_group_node(void *data)
{
    ...
    group_node *group = T_CONTAINER_OF(list, group_node, node);
    if (NULL != group->kv)
    {
...
static group_node *t_keyfile_find_group(const tkeyfile *keyfile, const tchar *name)
{
    ...
    {
        group = T_CONTAINER_OF(node, group_node, node);
 ...
 */
static void hash_string_foreach(void *data, void *userdata)
{
    T_ASSERT(NULL != data);
    ...
    kv_node *kv = T_CONTAINER_OF(string_node, kv_node, node);
    if (NULL != kv_data->kv_func)
    {
        kv_data->kv_func(kv->node.key, kv->value, kv_data->userdata);
    }
}
        ...
        tkey_value_func kv_func, void *userdata)
{
    ...
    {
        group = T_CONTAINER_OF(list_node, group_node, node);
    ...
    if (NULL != group)
    {
        ...
        if (NULL != group->kv)
        {
            t_hash_string_foreach(group->kv, hash_string_foreach, &data);
        }
    }
}
