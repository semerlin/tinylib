 ...
 * @return hash value
 */
static tuint32 t_hash_string_hash(const thash_string *hash_string, const tchar *key)
{
    tuint32 hash_val = 0;
    while (*key != '\0')
    {
    ...
    if (NULL != new_hash_string)
    {
        ...
        for (index = 0; index < hash_string->table_size; ++index)
        {
            ...
            while(hlist_node != NULL)
            {
                ...
                string_node = T_CONTAINER_OF(hlist_node, thash_string_node, node);
 ...
 */
tint t_hash_string_init_node(thash_string_node *node, const tchar *key)
{
    T_ASSERT(NULL != node);
 ...
 */
thash_string_node *t_hash_string_remove(thash_string *hash_string, const tchar *key)
{
    T_ASSERT(NULL != hash_string);
    T_ASSERT(NULL != key);
 ...
 * @return hash node
 */
thash_string_node *t_hash_string_get(const thash_string *hash_string, const tchar *key)
{
    T_ASSERT(NULL != hash_string);
    T_ASSERT(NULL != key);
 ...
 */
void t_hash_string_keys(const thash_string *hash_string, tchar **keys)
{
    T_ASSERT(NULL != hash_string);
    T_ASSERT(NULL != keys);
...
void t_hash_string_foreach(const thash_string *hash_string, thash_func hash_func, void *userdata)
{
    ...
    for (tuint32 i = 0; i < hash_string->table_size; ++i)
    {
        t_hlist_foreach(hlist_node, &hash_string->head[i])
        {
            ...
            if (NULL != hash_func)
            {
                hash_func(string_node, userdata);
            }
        }
    }
}
 ...
 */
tbool t_hash_string_contain(const thash_string *hash_string, const tchar *key)
{
    T_ASSERT(NULL != hash_string);
    T_ASSERT(NULL != key);
 ...
 */
void t_hash_string_free(thash_string *hash_string, tgeneric_func free_func)
{
    T_ASSERT(NULL != hash_string);
    ...
    for (tuint32 i = 0; i < hash_string->table_size; ++i)
    {
        ...
        while (cur != NULL)
        {
            ...
            if (NULL != free_func)
            {
                free_func(T_CONTAINER_OF(cur, thash_string_node, node));
            }
            cur = temp;
        }
    }
 ...
 * @param free_func - resource free function
 */
void t_hash_string_clear(thash_string *hash_string, tgeneric_func free_func)
{
    T_ASSERT(NULL != hash_string);
